...
pub fn parse(input: &str) -> anyhow::Result<(u16, u16, Vec<SchmGame>)> {
    ...
    for entry in entries{
        ...
        let fulltitle = get_first_child_text(entry, "title").map(|s| s.to_owned()).ok().unwrap();
...
pub async fn get_list(url: &str, filter_level: &str, page_number: u16) -> anyhow::Result<String>{
    let client = reqwest::Client::builder()
        .user_agent("Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36")
        .build()?;

    let url = format!(
        "{url}?methodName=FindGames&Names=Locale&Values=en-US&Names=LegalLocale&Values=en-US&Names=Store&Values=1&Names=PageSize&Values=100&Names=PageNum&Values={page_number}&Names=DetailView&Values=3&Names=AvatarBodyTypes&Values=3&Names=AvatarBodyTypes&Values=1&Names=UserTypes&Values=2&Names=UserTypes&Values=3&Names=MediaTypes&Values=1&Names=MediaTypes&Values=21&Names=MediaTypes&Values=23&Names=ImageFormats&Values=4&Names=ImageFormats&Values=5&Names=ImageSizes&Values=14&Names=ImageSizes&Values=15&Names=ImageSizes&Values=23&Names=OrderBy&Values=1&Names=OrderDirection&Values=1&Names=OfferFilterLevel&Values={filter_level}&Names=CategoryIds&Values=3027"
    );
...
async fn main() -> anyhow::Result<()>{
    ...
    for filter_level in filter_level{
        let mut quantity_to_fill: u16 = 1;
        let mut page_number: u16 = 1;
        let mut first_time = true;

        while quantity_to_fill > 0 {
            let input = get_list(&url_env, filter_level, page_number).await?;
            ...
            println!("Filling: {quantity_to_fill} - TOTAL: {total_items}");

            let wait_secs = rand::rng().random_range(1..=4);
